...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/14 17:15:37 by dde-carv          #+#    #+#             */
/*   Updated: 2025/04/14 17:19:44 by dde-carv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_args(char **av)
{
	...
	if (av[5] && (ft_atoi(av[5]) <= 0 || !ft_isnumber(av[5])))
		return (ft_putendl_fd("Invalid number of times each philosopher \
			must eat!", 2), 0);
	return (1);
}
...
[number_of_times_each_philosopher_must_eat]", 2), 1);
	...
	philo_clean(NULL, &program, forks);
	return (0);
}
